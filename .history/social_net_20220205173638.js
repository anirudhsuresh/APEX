...
function UpdateEmailNet() {
  ...
  function mouseover(d) {
    d3.select(this).delay("100").duration("200").attr("font-weight", "bold"); // make the current dev bold
    d3.select(this).delay(1).select("text").style("fill", "tomato");
    d3.select(this).delay(1).select("text").style("font-size", "17px");
    bp.mouseover(d);
    g.selectAll(".mainBars")
      .select(".perc")
      .text(function (d) {
